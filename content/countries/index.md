---
title: Canadian Representative for Manufacturers Outside Canada, by Country
description: Country-specific notes for manufacturers in China, Taiwan, South Korea, Japan, the United States, Germany, the United Kingdom and India appointing an ISED Canadian Representative: CB workflows, payment, time zones and mutual recognition.
h1: Manufacturers outside Canada, by country
short: Countries
type: page
updated: 2026-08-27
---

<div class="answer" markdown="1">
Any applicant with a company address outside Canada needs a Canadian Representative under RSP-100 section 4.1, wherever it is based. The pages below cover what tends to differ in practice by country: which certification bodies and labs are usually involved, whether a mutual recognition arrangement lets a local CB issue ISED certificates, how payment and time zones work with us, and the questions we hear most from that market. If your country is not listed, the process is identical; [order here](/quote/).
</div>

<!--list-->

## Manufacturers elsewhere

We accept applicants from any country outside Canada. The same letter, fee and turnaround apply, and revisions are free with no time limit. Payment by card in US dollars is simplest; wire transfers are accepted from any bank.
